...
-- params: helpers module, xml_type ("ui" or "settings")
-- returns: directory path, filename
function M.resolve_xml_file(helpers, xml_type)
  ...
  if xml_type == "ui" then
    ...
    content = M.UI_XML_CONTENT
  elseif xml_type == "settings" then
    filename = "TimeMAtricks_Settings_UI.xml"
    content = M.UI_XML_SETTINGS
  else
    ErrPrintf("Unknown XML type: " .. tostring(xml_type))
    return nil, nil
  end
  ...
  if not FileExists(full) then
    local ok = helpers.write_text_file(full, content)
    if not ok then
      full = (base .. "/" .. filename)
      helpers.write_text_file(full, content)
      dir = base
    end
  end
